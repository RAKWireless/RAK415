...
#define    RAK_COPY_UCONFIG_CMD                   "at+copy_cfg"				 //拷贝当前用户参数到出厂参数

/***************AP STA查询操作命令 ****************/
#define    RAK_QUERY_CONSTATUS_CMD                "at+con_status"
#define    RAK_QUERY_IPCONFIG_CMD                 "at+ipconfig"
#define    RAK_QUERY_TCPSTATUS_CMD                "at+tcp_status=0"			//查询当前TCP连接（单socket或SocketA）
...
#define    RAK_EASY_CONFIG_CMD                    "at+easy_config"	 	     //进入一键配置
#define    RAK_WPS_CONFIG_CMD                     "at+wps"			         //进入WPS配置
	
						
                

/****************AP STA查询操作命令 ***************/
#define    RAK_SEND_DATA_CMD         	    "at+send_data="		  // at+send_data=0,10,1234567890 通过单socket或SocketA 发送10个字节长度数据
																  // at+send_data=1,10,1234567890 通过SocketB 发送10个字节长度数据
#define    RAK_RECIEVE_DATA_CMD             "at+recv_data="		  // 模块主动返回给主机
                                                                  // at+recv_data=1B(socketID)2B(对方端口)4B(对方IP)2B(数据长度)nB()数据部分
																  //				0            25000    192.168.1.101  10     1234567890
																  // 模块SocketA接收到从IP地址192.168.1.100和端口号12345发来的字节长度为10的数据：1234567890
#define    SOCKETA_HDER			    "S0"
#define    SOCKETB_HDER			    "S1"
//模块进入透传模式后  双Socket开启时  数据部分开头都是S0和S1  表示从SocketA或SocketB  发送或接收
